 ...
 *
 * @file Central logging orchestrator that merges backend and frontend log events into a unified
 * output with correct timestamp ordering. Uses a reorder buffer with dynamic delay to compensate
 * for asynchronous IPC delivery of frontend events.
 *
 * Backend events flow through the normal appender chain via handle().
 * Frontend events arrive directly via handleFrontendEvent() from the IPC listener.
 * On flush, events are delegated to all registered child appenders (e.g. Console + File).
 *
 * Configurable via `configureLogging` — child appenders are instantiated from a nested
 * `delegates` config object via a setter that mirrors configureLogging's own instantiation logic.
 *
 ...
 *
 * Registered as the sole appender on the root logger. Backend events arrive via the normal
 * `handle()` path, frontend events via `handleFrontendEvent()`. Both are buffered, sorted by
 * timestamp, and flushed to all registered delegate appenders with an origin prefix
 * ("Backend : " / "Frontend: ") prepended to callSite.file.
 */
export class PipelineAppender implements IAppender {
  ...
  /**
   * Setter for nested delegate appender configs. Called by `configureLogging` via `Reflect.set`.
   * Instantiates each delegate from its config, applying properties the same way configureLogging
   * handles top-level appender configs.
   */
  ...
  /**
   * Receive a backend event from the normal appender chain.
   * Inserts into the buffer and starts/resets the max-delay timer.
   */
  ...
  /**
   * Receive a frontend event directly from the IPC listener.
   * Inserts into the buffer and flushes all events with timestamps <= this event's timestamp,
   * because no earlier frontend event can still be in transit.
   */
  ...
  /**
   * Prepare a single buffered event (shorten path, prepend origin prefix)
   * and delegate to all registered child appenders.
   */
  ...
  /**
   * Shorten a file path to its meaningful part so the origin prefix fits within the
   * column width used by AbstractBaseAppender's truncateOrExtendLeft(file, 50).
   *
  ...
  /**
   * Cancel any pending flush timer and start a new one with `maxDelay` duration.
   * Called after each backend event to ensure the buffer is eventually flushed
   * even when no frontend event arrives.
   */
